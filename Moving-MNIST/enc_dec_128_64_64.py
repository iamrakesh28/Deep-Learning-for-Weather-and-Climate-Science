...
class Decoder(tf.keras.Model):
    ...
    def __init__(self, dec_layers, unit_list, filter_sz):
        ...
        self.conv_nn    = tf.keras.layers.Conv2D(filters=16, 
                                                kernel_size=(1, 1),
                                                padding="same",
                                                activation='sigmoid',
                                                data_format="channels_last")
...
class EncoderDecoder:
    ...
    def predict(self, input_, output_seq):
        ...
        for t in range(output_seq):
            ...
            predictions.append(prediction.numpy().reshape(64, 64))
    ...
    def test_model(self, X, Y):
        e1 = self.evaluate(X[8000:9000], Y[8000:9000], True)
        e2 = self.evaluate(X[9000:], Y[9000:], False)
        y1 = self.predict(X[50], 10)
        y2 = self.predict(X[9000], 10)
        y3 = self.predict(X[9500], 10)
        y4 = self.predict(X[9345], 10)
        plot_result(X[50].numpy().reshape(10, 64, 64), Y[50].numpy().reshape(10, 64, 64), y1)
        plot_result(X[9000].numpy().reshape(10, 64, 64), Y[9000].numpy().reshape(10, 64, 64), y2)
        plot_result(X[9500].numpy().reshape(10, 64, 64), Y[9500].numpy().reshape(10, 64, 64), y3)
        plot_result(X[9345].numpy().reshape(10, 64, 64), Y[9345].numpy().reshape(10, 64, 64), y4)

    
def load_dataset(path, filename):
    data = np.load(path + filename)
    data = data.swapaxes(0, 1)
    data[[1005, 9000]] = data[[9000, 1005]]
    train_data = data
    # patch size 4 x 4
...
model = EncoderDecoder(3, [128, 64, 64], [(5, 5), (5, 5), (5, 5)], 32, (X.shape[2], X.shape[3]), './training_checkpoints')
#model.restore()
model.train(X[:8000], Y[:8000], 200, X, Y)
